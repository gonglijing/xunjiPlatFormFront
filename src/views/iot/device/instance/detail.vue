      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        ...
        <el-tab-pane label="日志管理" name="4">
          ...
          <el-table style="width: 100%" :data="logtableData.data">
            ...
            <el-table-column label="下发路由" min-width="180">
              <template #default="scope">
                <span>{{ getLogRouteText(scope.row) }}</span>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="300" align="center" fixed="right">
              <template #default="scope">
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const parseLogJson = (content: any) => {
    ...
    const getLogRouteText = (row: any) => {
      const parsed = row.__parsedContent || parseLogJson(row.content);
      const route = parsed?.route;
      if (!route) {
        return '-';
      }
      const channelDeviceKey = route.channelDeviceKey || '-';
      if (route.viaGateway) {
        const targetDeviceKey = route.targetDeviceKey || '-';
        return `经网关(${channelDeviceKey}) → 子设备(${targetDeviceKey})`;
      }
      return `直连(${channelDeviceKey})`;
    }

    const onLogDetail = (row: any) => {
      const parsed = row.__parsedContent || parseLogJson(row.content);
      state.jsonData = parsed || { rawContent: row.content };
    ...
    const getlog = () => {
      ...
      api.instance.getLogList(state.logtableData.param).then((res: any) => {
        state.logtableData.data = (res.list || []).map((item: any) => ({
          ...item,
          __parsedContent: parseLogJson(item.content),
        }));
